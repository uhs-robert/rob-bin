...
print_header() {
  ...
  local length=${#header}
  local border
  border=$(printf "%-${length}s" " " | sed "s/ /─/g")
  echo ""
  printf '%s%s%s' "$BRIGHT_YELLOW" "$header" "$ENDL"
  printf '%s%s%s' "$BRIGHT_YELLOW" "$border" "$ENDL"
}
...
print_title() {
  ...
  local BORDER_LEN=$(((WIDTH - TITLE_LEN - 2) / 2))
  local BORDER
  BORDER=$(printf "%-${BORDER_LEN}s" " " | sed "s/ /─/g")
  printf '%s%s %s %s %s' "$CYAN" "$BORDER" "$TITLE_UPPER" "$BORDER" "$ENDL"
}
...
check_dependency() {
  command -v "$1" >/dev/null 2>&1 || {
    echo >&2 "$(echo_red "Error: $1 is not installed. Please install it.")"
    exit 1
  }
}
